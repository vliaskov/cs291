...
function createCapsule( material, radius, top, bottom, segmentsWidth, openTop, openBottom )
{
    ...
    if (!openBottom) {
        var botsph = new THREE.Mesh( sphGeom, material);
        //botsph.MatrixAutoUpdate = false;
        //botsph.matrix.makeTranslation(bottom.x, bottom.y, bottom.z);
        botsph.position = new THREE.Vector3( top.x, top.y, top.z);
        capsule.add( botsph );    
    }
    if (!openTop) {
        var topsph = new THREE.Mesh( sphGeom, material);
        //topsph.MatrixAutoUpdate = false;
        //topsph.matrix.makeTranslation(top.x, top.y, top.z);
        topsph.position = new THREE.Vector3(top.x, top.y, top.z);
